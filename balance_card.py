...
def generate_balance_card(user_id, username, display_name, balance, casino_name="THE CASINO", avatar_url=None):
    ...
    # Header
    draw.text((40, 30), str(casino_name).upper(), font=font, fill=(148, 163, 184))
    ...
    if avatar_url:
        try:
            ...
            print(f"⚠️ Image paste skipped: {img_err}")

    # Text details
    text_x = 150 if avatar_url else 40
    clean_name = str(display_name).encode('ascii', 'ignore').decode('ascii').strip() or "Player"
    ...
    draw.text((70, 280), f"RS. {balance:,.2f}", font=font, fill=balance_color)
