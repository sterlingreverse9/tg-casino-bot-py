...
def generate_balance_card(user_id, username, display_name, balance, casino_name="THE CASINO", avatar_url=None):
    # Compact & Focused Aspect Ratio (Width: 800, Height: 500)
    width, height = 800, 500
    card = Image.new("RGBA", (width, height), (15, 23, 42))
    draw = ImageDraw.Draw(card)

    # Fonts Setup with Fallback Sizes
    try:
        font_casino = ImageFont.truetype("arialbd.ttf", 28)
        font_name = ImageFont.truetype("arialbd.ttf", 32)
        font_user = ImageFont.truetype("arial.ttf", 22)
        font_bal_label = ImageFont.truetype("arialbd.ttf", 24)
        font_bal_val = ImageFont.truetype("arialbd.ttf", 85)
    except IOError:
        font_casino = font_name = font_user = font_bal_label = font_bal_val = ImageFont.load_default()

    # --- 1. CASINO HEADER (Top Left) ---
    ...
    # Clean display name (strips unparseable emojis if any)
    ...
    # --- 3. HUGE BALANCE SECTION (Occupies Most Space) ---
    # Container Box
    ...
    # Formatting as RS. for bulletproof cross-platform font rendering
    formatted_bal = f"RS. {balance:,.2f}"
    draw.text((70, 290), formatted_bal, font=font_bal_val, fill=balance_color)
