...
async def process_message(client, message):
    ...
    for _ in range(len(clients)):
        ...
        try:
            ...
            print(
                f"Rate limited. Rotating account... Waiting {e.value}s on this instance."
            )
...
async def group_listener(cli, msg):
    ...
    if chat_identifier in target_groups:
...
async def start_promo_engine():
    ...
    if not clients:
        print(
            "No userbot accounts loaded. Add an account using /promote in DM first."
        )
        return
    ...
    # Join target groups and bind event handlers dynamically to each client
    for cli in clients:
        ...
        # Dynamically attach message handler to running Pyrogram instance
        cli.add_handler(
